...
2. Bulk edge growth: grows coating outward from the surface with remaining
   volume determined by dp_dc_ratio.
"""
...
def apply_potential_edge_coating(
    ...
    """Apply potential-based edge-growing coating.

    Stage 1 ensures surface coverage. Stage 2 grows outward.
    ...
    # Edge centers for potential calculation
    edge_mask_flat = surface_3d.ravel()
    ...
    # 5. Stage 1: Select surface void voxels using edge potential
    ...
    dists = cdist(surface_void_centers, edge_centers)
    ...
    # 7. Stage 2: Fill remaining volume with edge-growing potential
    ...
    bulk_dists = cdist(bulk_void_centers, edge_centers)
