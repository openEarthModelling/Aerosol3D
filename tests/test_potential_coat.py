...
class TestPotentialCoating:
    def test_coating_added(self, soot_material, sulfate_material):
        ...
        from aerosol3d.modeling.potential_coat import apply_potential_coating

        p = AerosolParticle(name="test", unit="nm")
        core = create_sphere(center=(0, 0, 0), radius=50.0)
        p.add_mesh("bc_core", core, soot_material)

        result = apply_potential_coating(
            p, target_f_bc=0.5, material=sulfate_material, resolution=32)
    ...
    def test_f_bc_validation(self, soot_material, sulfate_material):
        ...
        with pytest.raises(ValueError):
            apply_potential_coating(
                p, target_f_bc=1.5, material=sulfate_material)
